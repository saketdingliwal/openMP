...
double sign(point p,point p1,point p2)
{
	return 1.0 * (p.y-p1.y)*(p2.x-p1.x) - 1.0 *(p2.y-p1.y) * (p.x - p1.x);
}
...
int main()
{
	...
	while(comp_list.size()!=0)
	{
		...
		{	
			...
			for(int j=0;j<comp_list[i].v.size();j++)
			{
				double dist = pnt_in_eqn(comp_list[i].v[j],comp_list[i].p1,comp_list[i].p2);
				// cout << j << " " << dist<< ",";
				if(maxx < dist)
					p = comp_list[i].v[j];
				maxx = max(maxx,dist);
			...
			else
			{
				...
				for(int j=0;j<comp_list[i].v.size();j++)
				{
					double sign1 = sign(comp_list[i].v[j],c1.p1,c1.p2) * sign(comp_list[i].p2,c1.p1,c1.p2);
					if(sign1<0)
						c1.v.push_back(comp_list[i].v[j]);
					double sign2 = sign(comp_list[i].v[j],c2.p1,c2.p2) * sign(comp_list[i].p1,c2.p1,c2.p2);
					if(sign2<0)
